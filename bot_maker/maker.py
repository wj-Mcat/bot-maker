...
from bot_maker.schema import *
from bot_maker.nlu import NLUServer 
from wechaty_puppet import FileBox


class Task:
    def __init__(self, nlu: NLUServer, user: Optional[User] = None, bot: Optional[Bot] = None):
        self.bot = bot or Bot()
        self.user = user or User()

        self.nlu_server: NLUServer = nlu
    ...
    async def wait_for_user(self):
        ...
        # message: Message = await self.nlu_server.parse(text)
    ...
    async def activate(self, msg: str):
        message = await self.nlu_server.parse(msg)
...
class Maker:
    ...
    async def feed_message(self, msg: str, target_user: Union[Contact, Room]) -> Optional[str]:
        ...
        # 2. 创建对话脚本
        task_type: Type[Task] = await self.match_task(msg)
        if task_type:
